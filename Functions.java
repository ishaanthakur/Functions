...
public class A2 {
	...
	public static boolean isMidSame(String s) {
		...
		if (s != "") {
			...
			if (s.length() == 1 || s.length() > 0 && s.length() % 2 != 0 && s.charAt(k - 1) == s.charAt(k + 1)) {
	...
	public static String dupCaps(String s) {
		...
		if (s != "") {
			...
			for (int i= 0; i < s.length(); i++ ) {
				// Rebuild string with lower-case letters after upper-case letters

				ch= s.charAt(i);

				if (Character.isUpperCase(ch) && ch >= 'A' && ch <= 'Z') {

					g= g + "" + ch + "" + Character.toLowerCase(ch);
	...
	public static String nextLowerCase(String s) {
		...
		if (s != "") {
			...
			String g= "";
			// Replace lower-case letters in string with the next letter in the alphabet
			...
			for (int i= 0; i < s.length(); i++ ) {
				...
				if (Character.isLetter(ch) && ch >= 'a' && ch <= 'z') {
					...
					if (ch != 'z') {
						...
						// wrap-around so that z's are replaced by a's
	...
	public static boolean sortToSame(String s, String u) {
		...
		// convert each string to a char array to sort
		...
		char[] char2= u.toCharArray();

		Arrays.sort(char2);

		String unew= new String(char2);
		// compare the strings

		if (snew.equals(unew)) { return true; }
	...
	public static boolean isCatenated(String s, int k) {
		...
		if (s.length() >= 1) {
			...
			if (s.length() % k == 0) {
				...
				// create string of same length made up of the substring
